...
public class ProblemCannibals extends Problem {
    ...
    Set<Object> getSuccessors(Object state) {
    	//System.out.println("Getting valid states");
        ...
        successor_state.canArray[boatL] = 0;
    ...
    {   
        ...
        if(state.canArray[missL] > 3 || state.canArray[missR] > 3 || state.canArray[cannL] > 3 || state.canArray[cannR] > 3) return false;
		if(state.canArray[boatL] + state.canArray[boatR] > 1) return false;
        
        //Now, checking if cannibals out number missionaries
		if(state.canArray[missL] - state.canArray[cannL] < 0) return false;
		if(state.canArray[missR] - state.canArray[cannR] < 0) return false;
        //TODO
